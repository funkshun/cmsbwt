...
inline uint64_t rank64(uint64_t value) {
  ...
  ret = (ret & (uint64_t)0x3333333333333333) +
        ((ret >> 2) & (uint64_t)0x3333333333333333);
...
public:
  ...
  inline void setBit(uint64_t ind) { setBit64(this->ba, ind); }
  ...
  inline uint64_t rank(uint64_t pos) {
    ...
    // return this->index[pos >> 6] + rank64(this->ba[pos >> 6] << (~pos &
    // 0x3F)); up to but NOT including "pos"
    return this->index[pos >> 6] +
           rank64((this->ba[pos >> 6] << (~pos & 0x3F)) << 1);
  }
