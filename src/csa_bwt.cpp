...
void CSA_BWT::constructFMIndex(bool storeD) {
  ...
  for (int x = 0; x < VC_LEN; x++) {
    if (x != 0 || storeD)
      this->csa[x] = new BitArray(this->totalSize);
    else
      this->csa[x] =
          new BitArray(1); // this is just a dummy to get deleted later
  }
  ...
  for (uint64_t x = 0; x < VC_LEN; x++) {
    if (x != 0 || storeD)
      this->csa[x]->createIndex(this->startIndex[x]);
  }
}
