...
bwtRange BaseBWT::findIndicesOfStr(uint8_t *kmer, uint64_t kmerSize) {
  bwtRange ret;
  ret.l = 0;
  ret.h = this->totalSize;
...
vector<uint64_t> BaseBWT::countPileup_i(vector<uint8_t> seq,
                                        uint64_t kmerSize) {
  ...
  for (uint64_t x = 0; x < numCounts; x++) {
    ret[x] = this->countKmer(&seq[x], kmerSize) +
             this->countKmer(&revComp[seqLen - kmerSize - x], kmerSize);
  }
  return ret;
}
